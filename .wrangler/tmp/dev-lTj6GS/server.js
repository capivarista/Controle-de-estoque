...
var server_default = {
  async fetch(request, env3) {
    ...
    await createEstoqueTable(env3);
    if (url.pathname === "/estoque") {
      if (request.method === "GET") {
        return getEstoque(env3);
      } else if (request.method === "POST") {
        return addEstoque(request, env3);
      }
    }
    return new Response("Rota n\xE3o encontrada", {
      status: 404,
      headers
    });
  }
};
async function createEstoqueTable(env3) {
  try {
    await env3.DB.prepare(`
        ...
        `).run();
    console.log("Tabela 'estoque' criada ou j\xE1 existe.");
  } catch (error3) {
    console.error("Erro ao criar tabela estoque:", error3);
  }
}
__name(createEstoqueTable, "createEstoqueTable");
async function getEstoque(env3) {
  try {
    const { results } = await env3.DB.prepare("SELECT * FROM estoque").all();
    return new Response(JSON.stringify(results.length ? results : { message: "Nenhum item no estoque" }), {
      status: 200,
      headers: { "Content-Type": "application/json" }
    });
  } catch (error3) {
    console.error("Erro ao buscar estoque:", error3);
    return new Response(JSON.stringify({ error: "Erro ao buscar estoque" }), { status: 500 });
  }
}
__name(getEstoque, "getEstoque");
async function addEstoque(request, env3) {
  try {
    const data = await request.json();
    if (!data.nome || !data.modelo || !data.preco || !data.quantidade) {
      return new Response(JSON.stringify({ error: "Todos os campos s\xE3o obrigat\xF3rios!" }), { status: 400 });
    }
    await env3.DB.prepare(`
...
// .wrangler/tmp/bundle-iPIEj5/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-iPIEj5/middleware-loader.entry.ts
