...
// .wrangler/tmp/bundle-nWD6XK/checked-fetch.js
...
var server_default = {
  async fetch(request, env) {
    ...
    const headers = {
      ...
      // Permitir qualquer origem (você pode limitar isso a um domínio específico)
...
async function getEstoque(env) {
  try {
    ...
    if (results.length === 0) {
      return new Response(JSON.stringify({ message: "Nenhum item no estoque" }), {
        status: 200,
...
async function addEstoque(request, env) {
  try {
    ...
    await database.DB.prepare(`
      ...
      // Usando a data atual para aquisição
    ).run();
...
// .wrangler/tmp/bundle-nWD6XK/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-nWD6XK/middleware-loader.entry.ts
